...
#   bash install.sh --setup-zai                    # register Zai MCP (not yet public)
# ============================================================================
...
ZAI_MCP_REPO="${ZAI_MCP_REPO:-https://github.com/anombyte93/zai-mcp.git}"
...
setup_zai_mcp() {
    ...
    warn "Note: zai-mcp is not yet publicly released. This setup will fail unless"
    warn "you have access to the private repository."
    ...
    if [[ -d "${zai_dir}" ]] && [[ -f "${zai_dir}/build/index.js" ]]; then
        ...
        git clone --depth 1 --quiet "${ZAI_MCP_REPO}" "${zai_dir}" 2>/dev/null \
            || { warn "Failed to clone Zai MCP repository (is ${ZAI_MCP_REPO} accessible?). Skipping Zai setup."; return 1; }
        (cd "${zai_dir}" && npm install --silent 2>/dev/null && npm run build --silent 2>/dev/null) \
            || { warn "Failed to build Zai MCP server. Skipping Zai setup."; return 1; }
...
MCPEOF
        ...
        # Insert zaiMCP entry using python for safe JSON manipulation
        # Pass api_key via env var to prevent code injection from untrusted input
        Z_AI_API_KEY_VALUE="${api_key}" python3 -c "
import json, os
...
config['mcpServers']['zaiMCP'] = {
    'command': 'node',
    'args': [zai_dir + '/build/index.js'],
    'env': {'Z_AI_API_KEY': os.environ['Z_AI_API_KEY_VALUE']}
}
...
main() {
    ...
    case "${args[0]:-}" in
        ...
        --help|-h)
            ...
            printf "  --setup-zai        Register Zai MCP server (not yet public — requires private access)\n"
