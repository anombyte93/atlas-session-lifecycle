...
GOVERNANCE_CACHE = Path.home() / ".claude" / "governance-cache.json"
...
def cmd_hook_stop(args):
    ...
    try:
        ...
        # Parse lifecycle state (JSON format)
        try:
            state = json.loads(state_content)
        except json.JSONDecodeError:
            # Legacy YAML-ish format fallback
            state = {}
            if 'active: false' in state_content:
                state["active"] = False
            elif 'active: true' in state_content:
                state["active"] = True
            for line in state_content.split('\n'):
                stripped = line.strip()
                if stripped.startswith('soul_purpose:'):
                    state["soul_purpose"] = stripped.split('soul_purpose:', 1)[1].strip().strip("'")
        ...
        purpose = state.get("soul_purpose", "")
...
def cmd_hook_activate(args):
    ...
    try:
        ...
        soul = args.soul_purpose.replace('\n', ' ').replace('\r', '').strip()[:500]
        # Use JSON format to avoid YAML escaping pitfalls
        content = json.dumps({
            "active": True,
            "soul_purpose": soul,
            "started_at": now
